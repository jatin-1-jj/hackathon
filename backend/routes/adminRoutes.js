...
router3.post("/verify/:userId", auth2, adminOnly, async (req, res) => {
  try {
    const user = await User3.findByIdAndUpdate(
      req.params.userId,
      { verified: true },
      { new: true }
    ).select("-password");
...
router3.post("/announcement", auth2, adminOnly, async (req, res) => {
  try {
    ...
    res.json({
      message: "Announcement created (no persistent model in this scaffold)",
      title,
      body,
    });
  } catch (err) {
...
router3.get("/memberships", auth2, adminOnly, async (req, res) => {
  try {
    const memberships = await Membership3.find().populate(
      "userId",
      "name email"
    );
