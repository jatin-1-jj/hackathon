...
router2.post('/profile', authMiddleware, async (req, res) => {
try {
const payload = req.body;
payload.userId = req.user._id;  


const alumni = await Alumni.findOneAndUpdate({ userId: req.user._id }, payload, { new: true, upsert: true });
...
try {
const alumni = await Alumni.findOne({ userId: req.user._id }).populate('userId', '-password');
res.json(alumni);
} catch (err) {
console.error(err);
res.status(500).json({ error: 'Server error' });
...
router2.get('/search', authMiddleware, async (req, res) => {
try {
