    ...
    useEffect(() => {
        const uniqueUsername = async () => {
            if (isValidUsername(username)) {
                const res = await checkUniqueUsername({ username, role });
        ...
        <div className="max-w-md mx-auto bg-white p-6 rounded shadow">
            ...
            <form onSubmit={submit} className="space-y-3">
                ...
                <h3 className='text-red-400'>
                    <span className='font-bold text-black'>NOTE:-</span>If not verified within 7-days account will be deleted automatically
                </h3>
            </form>
        </div>
    )
